...
As a user, I would like to check acceptance criteria of Nopcommerce topmenu
 ...
 Scenario: Verify that user can navigate to Books category
  ...
  Scenario: Verify that users can see 'Sort by' filter on Book Category page
  ...
  Scenario: Verify that user can see 'Name : A to Z' first option in 'Sort by' filter
   Given  User is on Books category page
   When   User is on Books category page
   And    User is on Books catergory pageClick on 'Sort by' Filter
   And    Click on 'Sort by' Filter
